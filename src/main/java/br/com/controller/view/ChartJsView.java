...
public class ChartJsView implements Serializable {
	...
	private static final long serialVersionUID = 8628321079072323524L;
	...
	DuppagDAO daoDuppag = new DuppagDAO();
	...
	public void createCartesianLinerModel() throws ClassNotFoundException, SQLException {
	    cartesianLinerModel = new LineChartModel();
	    ChartData data = new ChartData();

	    LineChartDataSet dataSet = new LineChartDataSet();

	    List<String[]> cr = daoDuprec.lerDuprecTresMeses(arquivoUtil.obterEmpresaDaSessao().getUsu());
	    List<String[]> cp = daoDuppag.lerDuppagTresMeses(arquivoUtil.obterEmpresaDaSessao().getUsu());

	    List<Object> values = new ArrayList<>();
	    List<Object> values2 = new ArrayList<>();
	    List<String> labels = new ArrayList<>();

	    // Adiciona os dados e rótulos das contas a receber
	    for (String[] strings : cr) {
	        values.add(strings[1]);
	        labels.add(strings[0]);
	    }

	    // Adiciona os dados e rótulos das contas a pagar
	    for (String[] strings : cp) {
	        values2.add(strings[1]);
	    }

	    dataSet.setBackgroundColor("#836FFF50");
	    dataSet.setBorderColor("#836FFF20");
	    dataSet.setData(values);
	    ...
	    dataSet2.setBackgroundColor("#EE82EE50");
	    dataSet2.setBorderColor("#EE82EE20");
	    dataSet2.setData(values2);
	    dataSet2.setLabel("Contas a Pagar");
	    dataSet2.setYaxisID("left-y-axis"); // Compartilha o mesmo eixo Y
	    ...
	    Title title = new Title();
	    title.setDisplay(true);
	    title.setText(arquivoUtil.obterEmpresaDaSessao().getNom());
	    options.setTitle(title);

	    cartesianLinerModel.setOptions(options);
	}

	public void createCartesianLinerModelGrupo() throws ClassNotFoundException, SQLException {
	    cartesianLinerModel = new LineChartModel();
	    ChartData data = new ChartData();

	    List<Object> summedValues = new ArrayList<>();
	    List<String> labels = new ArrayList<>();
	    
	    List<Empresas> empresas = daoEmpresas.listarEmpresasPorUsuario(arquivoUtil.obterUsuarioDaSessao().getCodigo());
	  
	    for (Empresas emp : empresas) {
	        List<String[]> cr = daoDuprec.lerDuprecTresMeses(emp.getUsu());

	        List<Object> values = new ArrayList<>();
	        
	        for (String[] strings : cr) {
	            values.add(Double.parseDouble(strings[1])); // Convertendo para double
	        }

	        // Somando os valores
	        if (summedValues.isEmpty()) {
	            summedValues.addAll(values);
	        } else {
	            for (int i = 0; i < values.size(); i++) {
	                summedValues.set(i, (double)summedValues.get(i) + (double)values.get(i));
	            }
	        }

	        // Adicionando os rótulos apenas uma vez
	        if (labels.isEmpty()) {
	            for (String[] strings : cr) {
	                labels.add(strings[0]);
	            }
	    ...
	    dataSet.setBackgroundColor("#836FFF50");
	    dataSet.setBorderColor("#836FFF20");
	    dataSet.setData(summedValues);
	    dataSet.setLabel("Total de Contas a Receber");
	    dataSet.setYaxisID("left-y-axis");
	    dataSet.setFill(true);
	    dataSet.setTension(0.5);
