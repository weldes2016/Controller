...
public class CliforDAO extends GenericDAO<Clifor> implements Serializable {

	private static final long serialVersionUID = 5887269097927959878L;

	public List<Clifor> listarClientesDirectus(String codigo) throws SQLException, ClassNotFoundException {
	...
	public void updateClientesGalaxPay(Duprec duprec, String id_galaxPay, String codigo) throws SQLException, ClassNotFoundException {
		CliforDAO dao = new CliforDAO();
		
		Clifor cli = dao.buscarCliente(duprec.getCliente(), codigo);
		
		System.out.println(cli.getCpfcgc());
		
		StringBuffer sql = new StringBuffer();
		System.out.println("Update Cliente codigo cliente: "+duprec.getCodcli()); 
		sql.append("update clifor set clifor.id_galaxpay = ? where clifor.id_clifor = ?");

		Connection conexao = ConexaoFactory.conectarMySql(codigo);
		PreparedStatement comando = conexao.prepareStatement(sql.toString());
		comando.setString(1, id_galaxPay);
		comando.setString(2,  duprec.getCodcli().getCod());
